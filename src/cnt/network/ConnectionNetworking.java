...
public class ConnectionNetworking
{
    ...
    /**
     * <p>Constructor with non-standard port</p>
     * <p>
     ...
     * @throws IOException Thrown in case of network errors.
     */
    public ConnectionNetworking(int port) 
    {
	// Check if public ip is same as internal ip
	// The IP service only want us to check every 300 sec. So getExternalIP should make sure to execute the check only every 300 sec. Use local cache instead of constant lookup.
	if (!getExternalIP().equals(getInternalIP()))
     ...
     */
    public Inet4Address externalIP;  //QUESTION: is really IPv4 required, doesn't IPv6 work as well?
    
    /**
     * Local ip. Not as probable as Public IP, but same reason.
     */
    public Inet4Address internalIP;  //QUESTION: is really IPv4 required, doesn't IPv6 work as well?
	...
	final Thread serverThread = new Thread(new TCPServer(_server, this));
	serverThread.setDaemon(true);
	serverThread.start();
    }
    
    /**
     * Setup the client to be running localy. Without PublicIP or NAT.
     */
	...
	if (this.connections != null && this.connections.isEmpty() == false)
	{
	    ...
	    for (final int peer : this.connections.keySet())
		try
		{
     ...
     * @return internal ip
     */
    private final Inet4Address getInternalIP()  //QUESTION: is really IPv4 required, doesn't IPv6 work as well?
    {
	try
	{
     ...
     * @return external ip
     */
    private Inet4Address getExternalIP()  //QUESTION: is really IPv4 required, doesn't IPv6 work as well?
    {
